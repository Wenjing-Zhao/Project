...
def draw_show():
    ...
    for j in range (brickheight):
       for i in range(brickwidth):
           if (bricks[j*brickwidth+i].active == False and bricks[j*brickwidth+i].colour=='r'):
               count+=1
               if bricks[j*brickwidth+i].continuehit:
                    score+=400
               else:
                    score+=200
    ...
    fill(255)
    text("Current Score: " + nf(score, 0, 0) , 420, 20)
    if maxscore<score:
        maxscore=score
    text("Highest Score: " + nf(maxscore, 0, 0) , 220, 20)
...
def draw():
    ...
    global PannelMove
    global continuehit
    if ball.update(pannelX,height * 0.9,grey_frame_width,width - grey_frame_width,blue_grey_height + 8,pannel_red_width,pannel_height,PannelMove):
        continuehit=False
    ...
    for j in range (brickheight):
        for i in range(brickwidth):
            ...
            if bricks[j*brickwidth+i].hit_test(ball.pos_x,ball.pos_y-ball.radius):
                bricks[j*brickwidth+i].active=False
                bricks[j*brickwidth+i].continuehit=continuehit
                continuehit=True
                ball.speed_y=-ball.speed_y
                #print ("Bottom hit detection")
                continue
            # Top hit detection
            if bricks[j*brickwidth+i].hit_test(ball.pos_x,ball.pos_y+ball.radius):
                bricks[j*brickwidth+i].active=False
                ...
                continuehit=True
                ball.speed_y=-ball.speed_y
                #print ("Top hit detection")
                continue
            # Collision detection type 2:
            # Bottom right corner
            if bricks[j*brickwidth+i].active and ball.hit_test(bricks[j*brickwidth+i].pos_x + bricks[j*brickwidth+i].w, bricks[j*brickwidth+i].pos_y + bricks[j*brickwidth+i].h):
                bricks[j*brickwidth+i].active = False
                bricks[j*brickwidth+i].continuehit=continuehit
                continuehit=True
                ...
                continue
            # Bottom left corner
            if bricks[j*brickwidth+i].active and ball.hit_test(bricks[j*brickwidth+i].pos_x, bricks[j*brickwidth+i].pos_y + bricks[j*brickwidth+i].h):
                bricks[j*brickwidth+i].active = False
                bricks[j*brickwidth+i].continuehit=continuehit
                continuehit=True
                ball.speed_x, ball.speed_y = -ball.speed_y, -ball.speed_x
                ...
                continue
           # Top right corner
            if bricks[j*brickwidth+i].active and ball.hit_test(bricks[j*brickwidth+i].pos_x + bricks[j*brickwidth+i].w, bricks[j*brickwidth+i].pos_y):
                bricks[j*brickwidth+i].active = False
                bricks[j*brickwidth+i].continuehit=continuehit
                continuehit=True
                ball.speed_x, ball.speed_y = -ball.speed_y, -ball.speed_x
                ...
                continue
            # Top left corner
            if bricks[j*brickwidth+i].active and ball.hit_test(bricks[j*brickwidth+i].pos_x, bricks[j*brickwidth+i].pos_y):
                bricks[j*brickwidth+i].active = False
                bricks[j*brickwidth+i].continuehit=continuehit
                continuehit=True
                ball.speed_x, ball.speed_y = -ball.speed_y, -ball.speed_x
